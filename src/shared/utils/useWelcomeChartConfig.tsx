...
interface UseWelcomeChartConfigProps {
  series?: number;
  datums?: number;
  useR?: boolean;
...
export function useWelcomeChartConfig({
  series = 1,
  datums = 10,
  useR,
  show = [],
  ...
  snapCursor = true, }
  : UseWelcomeChartConfigProps, bankResources?: any[]) {
  ...
  const [state, setState] = React.useState({
    ...
    snapCursor,
    datums,
    data: makeDataFrom(dataType, series, datums, useR, bankResources),
  });
  ...
  }, [count, dataType, datums, series, useR, bankResources]);
...
function makeDataFrom(dataType: DataType, series: number, datums: number, useR?: boolean, bankResources?: any[]) {
  return [
    ...new Array(series || Math.max(Math.round(Math.random() * 5), 1)),
  ].map((d, i) => makeSeries(i, dataType, datums, useR, bankResources));
}

function makeSeries(i: number, dataType: DataType, datums: number, useR?: boolean, bankResourcesArr?: any[]) {
  const start = 0;
  ...
  const bankResources: BankResource[] = bankResourcesArr && bankResourcesArr.length > 0 && bankResourcesArr[0] ? bankResourcesArr[0] : undefined;
  ...
  return {
    ...
    data: [...new Array(length)].map((_, i) => {
      ...
      if (dataType === "ordinal") {
        // x = bankResources ? bankResources[i].name : `Banco ${start + i}`;
        x = `Banco ${start + i}`;
